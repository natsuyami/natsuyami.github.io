...
const FlowerLoader = (() => {
  ...
  class FlowerLoader {
    ...
    _animate() {
      ...
      timeline.add(TweenMax.fromTo(center, 1, {
        scale: 0
      }, {
        scale: 1,
        ease: Elastic.easeOut.config(1.1, .75)
      }), 0).add(TweenMax.staggerTo(leaves, 1, {
        scale: 1,
        ease: Elastic.easeOut.config(1.1, .75)
      }, .2), 0.3).add(TweenMax.to(leaves, 0.3, {
        scale: 1.25,
        ease: Elastic.easeOut.config(1.5, 1)
      })).add(TweenMax.to(this._element.querySelector(Selector.LEAVES), 1, {
        duration: 1.5,
        rotation: 360,
        ease: Expo.easeInOut
      }), 1.7).add(TweenMax.to(leaves, 0.5, {
        scale: 0,
        ease: Elastic.easeInOut.config(1.1, 0.75)
      })).add(TweenMax.to(center, 0.5, {
        scale: 0,
        ease: Elastic.easeInOut.config(1.1, 0.75)
      }), '-=0.37');
    }
